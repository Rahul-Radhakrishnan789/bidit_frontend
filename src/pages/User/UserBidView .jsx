...
const UserBidView = () => {
    ...
    const finalBid = bidData.bidders && bidData.bidders.length > 0 ? bidData.bidders[bidData.bidders.length - 1].amount : 0;
    const finalBidUser =
        bidData.bidders && bidData.bidders.length > 0 ? bidData.bidders[bidData.bidders.length - 1].userId?.username : null;
    // -------------
    ...
    const placeBid = async () => {
        if (amount > finalBid && amount > bidData?.basePrice) {
            ...
            if (amount <= finalBid) {
                setToaster(true);
                handleClick();
            } else if (amount <= bidData.basePrice) {
                setToaster(false);
                handleClick();
            }
        }
    ...
    const timer = () => {
        const intervalId = setInterval(() => {
            const endTimeMs = new Date(bidData?.startTime).getTime() + bidData?.auctionDuration * 3600000;
        ...
        <Maincontainer>
            ...
            <SubContainer sx={{ padding: { xs: 0, sm: "1rem" } }}>
                                ...
                                <CurretBid>
                                    ...
                                    >
                                        ...
                                        <Box className="right">
                                            ...
                                            <p style={{ margin: "0 0 16px 0" }}>base price : ₹ {bidData.basePrice}</p>
                                        </Box>
                                    </Box>
                                    <Box
                                        className="bid_place"
                                    ...
                                    >
                                        ...
                                        <Box className="agree-check">
                                            ...
                                            <Box className="check-content">
                                                <span>I agree to pay if I win this domain</span> An additional fee of ₹15.88
                                                will be added to the final purchase price for one year of registration for
                                                this .com domain.
                                            </Box>
                                        </Box>
                                    </Box>
                ...
                <Snackbar
                    ...
                    onClose={handleClose}
                    message={
                        toaster ? `place your bid more than the current bid` : `place your bid more than the base price`
                    }
                    action={action}
